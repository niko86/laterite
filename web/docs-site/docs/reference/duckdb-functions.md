...
The AGS4 dictionary ships _inside_ the extension, so there is nothing to download.
...
Two functions turn a one-shot read into a durable, indexed store. The extension
API cannot cleanly issue `CREATE TABLE` or `ATTACH` from inside a table
function, so both **emit DDL rather than running it**: each returns an ordered
`(seq, stmt)` script for you to execute.
