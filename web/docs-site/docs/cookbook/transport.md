...
=== "Python"
    ...
    intend to validate later, because nothing about it changes.
...
=== "Node"
    ...
    **Add encryption** with the same passphrase-based envelope:
...
=== "CLI"
    ...
    ratio + timing summary prints to stderr; here the round-trip is proved by
    reading a group back out of the restored file). `lat lock` / `unlock` add the
    age passphrase envelope. The passphrase is **never** a flag, because argv
    leaks into `ps` and shell history; the precedence is `--password-file <path>` →
    ...
    anywhere, with `transport.unlock` in Python/Node or with stock `age`.
