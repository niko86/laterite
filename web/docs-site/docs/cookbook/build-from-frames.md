...
=== "Python"
    ...
    **To get a `TRAN`:** state it with `tran=TranStamp(issue=…, date=…, producer=…,
...
=== "Node"
    ...
    `buildAgs4` takes a `Map` (or array) of `[code, rows]` entries, or an
    arrow-js `Table` per group. In the row form, each row is a plain object whose
    **keys are the AGS headings**. Group order is preserved, so put `PROJ` first. It returns the same
    ...
    `TRAN`; without them those gaps are reported as Rules 14/15/17. Emit is
    pure, so no DuckDB peer is needed.
