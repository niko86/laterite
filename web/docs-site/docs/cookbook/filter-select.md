...
=== "Python"
    ...
    across groups, reach for `.sql(...)` instead
    (see [SQL across groups](./sql-across-groups.md)).
...
=== "DuckDB"
    ...
    add more `read_ags(...)` calls and join, as
    [SQL across groups](./sql-across-groups.md) shows.
