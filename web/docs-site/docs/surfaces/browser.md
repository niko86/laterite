...
explore, and convert it, backed by a guarantee no server-side tool can make:
...
`LOCA` and see its samples and tests fan out together. That is one borehole's
record set without writing a query. There is also a SQL box: DuckDB-wasm fed by the wasm
