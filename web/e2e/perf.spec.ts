...
test.describe(RUN ? "perf (PERF=1, low-end emulation)" : "perf (skipped — set PERF=1)", () => {
  test.skip(!RUN, "opt-in: PERF=1 npx playwright test perf.spec.ts --workers=1");
  ...
  for (const rate of RATES) {
    test(`CPU ${rate}x`, async ({ browser }) => {
      ...
      const t1 = Date.now();
      await page.locator('input[type="file"]').setInputFiles(fixture("many_findings.ags"));
      await expect(page.getByText("✗").first()).toBeVisible({ timeout: 180_000 });
      ...
      await expect(page.getByText(/data rows/)).toBeVisible({ timeout: 300_000 });
      ...
      await expect(page.getByText("LOCA_ID").last()).toBeVisible({ timeout: 120_000 });
