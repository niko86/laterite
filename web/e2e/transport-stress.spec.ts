...
test.describe(
  RUN
    ? "transport stress (STRESS=1)"
    : "transport stress (skipped — set STRESS=1)",
  () => {
    test.skip(
      !RUN,
      "opt-in: STRESS=1 npx playwright test transport-stress.spec.ts --workers=1",
    );
    ...
    for (const rate of RATES) {
      test(`CPU ${rate}x`, async ({ browser }) => {
        ...
        for (const mb of SIZES_MB) {
          ...
          const total =
            ((r.compressMs as number) || 0) + ((r.encryptMs as number) || 0);
          console.log(
