...
| duckdb cells vs the extension's manifest | `repo:tools/check_duckdb_manifest.py` | the duckdb verbs [[modality-register]] names and the functions the extension registers cover each other, both ways, against a pinned copy of its own `functions.json`. duckdb is a separate repo, so its cells are the one row in that register hand-authored rather than reflected — a function renamed upstream leaves a cell naming nothing, one added appears in no cell, and neither was visible. Coverage is counted per FUNCTION, not per cell; the seven the register has no capability for at all are named and printed on every run rather than filtered away |
