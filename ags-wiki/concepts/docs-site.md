...
- **…and the crates.io twin, `docs-vs-released-crates` (#283).** #278 wired every
  ...
  **printed** per crate to separate a defect from ordinary tree-ahead drift —
  ...
  is reported as *unasked*, never
  failed: "not released yet" must not arrive looking like "the released README is
  broken".
- **Changelog page — deliberately static.** `reference/changelog.md` is a
  short hand-written page that points at the repo-root `CHANGELOG.md` and
  GitHub Releases rather than mirroring their text into the site; nothing
  derives it at build (the `gen-files` scripts are brand/groups/types/cli).
  Because it carries no version number or notes of its own, there is nothing
  on it to drift when a release lands.
