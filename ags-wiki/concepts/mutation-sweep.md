...
presentation crate, and the `laterite-ags4-parse` leaf (`lib.rs` + `scan.rs`)
are now swept; near-term Rust queue (tracks [[coverage-campaign]]'s): the
remaining engine crates — `laterite-ags4-core`, `laterite-ags4-validator`,
`laterite-ags4-emit`, `laterite-ags4-reference`.
