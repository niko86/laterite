...
ags_editions: []
repo_refs:
  cli_verbs: "repo:rust-packages/laterite-cli/tests/cli_verbs.rs"
...
The `lat`-verb CLI tests (`repo:rust-packages/laterite-cli/tests/cli_verbs.rs`)
...
| (whole crate) | laterite-ags4-emit | 2026-07-27 | 90 | 76 killed / 3 equivalent (11 unviable) | the shared emit orchestrator + byte writer. The crate already had solid tests (the whole `arrow_in` transpose, the emit round-trips); the gaps were concentrated in the orchestrator's helpers. Nine new tests close the default-build survivors: **emit.rs** — the Strict error gate (BOTH sides: the `severity == Error` count and `errors > 0`, via a complete clean fixture vs a duplicate-KEY error one — Strict does *not* synthesize the mandatory TRAN/UNIT/TYPE, so the clean fixture carries them); `resolve_meta`'s override-wins-else-fill guard (explicit UNIT wins, blank falls back to the dict); `format_cell` emitting a `DT` **string** verbatim (not through `ags4_str`, which would strip a `T00:00:00`); `collect_units` (PU-typed columns only + non-blank) and `collect_types` (blank + literal-`"TYPE"` exclusion); and `concatenator` (a group's `TRAN_RCON`, else the AGS `"+"`). **error.rs** — `EmitError`'s per-variant `Display` (a whole-`fmt` replacement empties every message). **writer.rs** — the `write_ags4`/`write_ags4_matrix` `i > 0` group separators (one blank BETWEEN groups, none before the first — a single-group test sees neither edge). `arrow_in` is `#[cfg(feature = "arrow")]`, so its 16 mutants are **cfg-false** in the default sweep and were scored honestly with `--features arrow` (12 killed by the existing arm/transpose tests). The 3 residuals are all **equivalent**: the `Utf8`/`LargeUtf8` `cell_value` arms (Arrow's `ArrayFormatter` fallback renders a string identically to the arm's `Value::String`), and `validate()`'s `dict_version` field (`check_parsed` validates with the passed edition-matched `dict` and never reads `opts.dict_version` — it is consumed only by the path-based `check_file`/`fix_document`) |
| (whole crate) | laterite-ags4-diff | 2026-07-30 | 51 | 43 killed / 0 residual (8 unviable) | Retro-sweep (laterite#127). **24 of 51 missed on the first pass — a 47% miss rate — and it was one structural hole, not scattered weak assertions: `diff_parsed`, the crate's ONLY public function and the entry point every surface uses (`lat diff`, Python, Node, wasm), had no test at all.** It appeared exactly once in the file: its own definition. The four existing tests all exercised the private helpers *underneath* it (`diff_group`, `changed_cells`), so line coverage looked respectable while the file-level union, the running totals and the include-predicate were never executed by a test. This is the case mutation testing exists for and line coverage structurally cannot see. All 24 classified **real gap** — no inert code, no equivalents. Six new tests: **`diff_parsed`** — the group union across both files (B's order then A-only, reaching the `!b.groups.contains_key` arm), totals summed across groups, an identical group correctly EXCLUDED, and a rows-identical/headings-differ group correctly INCLUDED (the second arm of the predicate — counting rows alone drops a schema-only change). **`diff_group`** — `headings_added` vs `headings_removed` asserted separately (inverting either filter is otherwise silent); the both-sides requirement on KEY headings (an `\|\|` there would build a row key from a heading half the rows lack); and `max_rows_per_group` asserted on BOTH halves — capped rows AND untruncated counts, which the doc comment promises and nothing tested, so a cap that also truncated the counts would have silently under-reported diffs. **A second pass was needed and is the lesson worth keeping:** the first re-sweep still left 6 survivors on the `total_added`/`total_removed` accumulators, because that test's totals were both **0** — and zero is a fixed point of `-=` and `*=` alike. Killing them needed a fixture where the counts are non-zero AND the operands differ: one group netting 1 added + 1 removed (vanishes under `added - removed`) and one netting 2 added + 0 removed (vanishes under `added * removed`), so each wrong operator drops a different group. A count only proves an operator when zero is not the witness. |
| (whole crate) | laterite-ags4-excel | 2026-07-30 | 89 | 74 killed / 1 residual (14 unviable) | Retro-sweep (laterite#127), completing the issue's stated scope. 15 missed on the first pass. **The headline is a non-falsifiable assertion in the existing round-trip test**: `bytes_round_trips_with_no_filesystem` asserted `LOCA_NATE == "523145.10"` on a fixture whose input was ALREADY `"523145.10"`, so it passed whether or not the UNIT/TYPE rows survived the sheet — `write_group_row` (which writes only those two rows) could be stubbed to `Ok(())` with every assertion still green. Replaced by a fixture where survival CHANGES the value (`523145.1` + a `2DP` TYPE → `523145.10` only if the TYPE row round-tripped), which kills the stub, the column-shift in that writer, and the reader's `"UNIT"`/`"TYPE"` match arms at once. **`column_width`** was wholly unasserted — replaceable by any constant — despite being documented python-ags4 parity (`min(max(13, max_len+1), 75)`); now pinned at the 13 floor, the 75 ceiling, and an exact value in the unclamped band (the only place `+ 1` is observable), plus a test that the UNIT/TYPE lookup uses the heading's OWN column index. **`format_sf`'s `i < 0` boundary**: at `i == 0` the arms are NOT interchangeable — `{:.0}` rounds half-to-even (matching python-ags4's `f"{v:.0f}"`) while the other arm's `.round()` rounds half away from zero, so 2SF of 12.5 is "12" not "13". **One survivor was nearly misclassified as equivalent and is worth the warning**: `numeric_prefix`'s `prefix.is_empty() \|\| !all_ascii_digits` guard looks redundant with the `parse::<usize>()` that follows, but Rust's integer `FromStr` **accepts a leading `+`** (`"+5".parse::<usize>()` is `Ok(5)`), so dropping the guard turns a malformed `"+5DP"` TYPE into a valid 5-decimal format. Verified with a scratch binary rather than by reasoning. **1 residual, deliberately recorded rather than forced**: `ags4_bytes_to_xlsx_with:164` applies column WIDTHS via `excel_col(i + 1)`; the `* ` mutant shifts every width one column left. calamine exposes no column-width accessor, so asserting it needs a new dev-dependency on a zip reader plus sheet-XML parsing — disproportionate for a display-only property whose *values* are now covered by the `column_width` unit tests. Only the index they land on is unasserted. |
| `src/ags4/fix.rs` + `src/ags4/build.rs` | laterite | 2026-08-05 | 81 | 48 killed / 0 residual (33 unviable) | **The facade's first sweep** — 23 rows before this one and not one for `laterite`, so phases 4a (`transport.rs`) and 4b (`cert.rs`) landed without this step. Phase 4c of dec-facade-parity. 5 of 48 viable missed on the first pass, all in `fix.rs`, all real gaps in the new tests rather than inert code. **The headline is a non-falsifiable assertion, the same shape 4a's zstd-level test had**: `fixable_rules()` survived being replaced with `vec!["xyzzy"]` because the test round-tripped every advertised label through `only()` — and an unrecognised label simply *selects nothing* and the run succeeds, so a list of nonsense passed exactly as well as the real one. Rewritten to compare the advertised set against the rule labels the fixer actually **emits**, over two fixtures, plus a shape check that each entry is a rule label rather than a word. The other four were the `Repair` record — the ledger a UI puts in front of someone: `label()` asserted only non-empty (now asserted on content), `line()` asserted `None` for the whole-file repair and never that a line-anchored one *has* a line, so hard-wiring `None` satisfied it (now both halves — `pad_short_row` is `Some(5)`, `normalize_crlf` is `None`), and its `Debug` was never rendered. Re-sweep clean: 48 caught, 0 missed. The 33 unviable are the builder methods, whose `self`-returning shape gives the mutator nothing to substitute. |
| `src/ags4/mod.rs` + `src/ags4/cert.rs` | laterite | 2026-08-06 | 77 | 39 killed / 0 residual (37 unviable) | Swept alongside the `fix.rs`/`build.rs` row above, because phases 4a and 4b had landed without a sweep and `mod.rs` is where 4b's real logic lives. 7 missed on the first pass (6 in `mod.rs`, 1 in `cert.rs`). **Two were in code that phase 4c itself introduced**: both non-default arms of the new `validator_kind` helper could be DELETED with the suite green, because the tests that look like they cover them reach the same `ErrorKind` by a different route entirely — `fix`'s missing-file error comes from this crate's own `fs::read`, and a bad `edition("4.9")` is caught by `resolve_edition` before the engine is ever asked. Now reached properly: `validate()` on a missing path for the engine's own `not_found`, and an **AGS3** file for `unsupported_edition`, which is the only way to make the engine emit it. The other five were **four `Debug` impls that could be stubbed out entirely** (`Read`, `Validate`, `Write`, `Certify`) plus `Written::fixes_applied` surviving replacement with `1`. Both are the same failure of assertion shape: `debug_never_prints_the_document` asserted only that the rendering did NOT leak contents, which a `Debug` printing nothing satisfies perfectly, and `> 0` accepts a hard-wired 1. Every case now asserts what the value IS as well as what it is not. Re-sweep clean. |
| `src/ags4/diff.rs` + `src/ags4/merge.rs` | laterite | 2026-08-06 | 134 | 109 killed / 0 residual (25 unviable) | Phase 4d of dec-facade-parity. **22 of 109 viable missed on the first pass — a 20% miss rate — and every one was an accessor whose test compared it against ITSELF.** Four shapes, worth naming because each reads as a real assertion: (1) `Merged::bytes` survived returning an empty slice, because `save` writes `bytes()` and the test compared the file on disk with `bytes()` — both sides move together, so it only proved the accessor agrees with itself; now compared against `text()`. (2) `Revision::winner` survived being replaced with the constant `1`, because the only merge under test had two files and the second won; now a three-file merge asserts the index MOVES to 2. (3) The `GroupChange` counts survived `0`/`1` substitution because the cap test compares a capped run with an uncapped one — a constant satisfies both — and because every fixture happened to remove exactly one row; the fixture now has its own three-borehole baseline so each count is ≥2 or distinct. (4) `keyed`, `headings_removed` and both `line_*` accessors had only ever been exercised on the side that returns the default: a keyed group, an ADDED heading, and no line assertions at all. A custom `ZZZZ` group the dictionary does not know supplies the unkeyed and heading-removed counterparts in one fixture. Two engine behaviours had to be discovered to write the merge tests and are now recorded in them: `X` absorbs a typed column SILENTLY, so an `X`-vs-`2DP` clash widens with no note and only two different non-`X` types (`2DP` vs `3SF`) produce one; and a note's WHERE lives in its `group`/`heading` fields, not in its message text. Re-sweep clean. |
| `src/transport.rs` | laterite | 2026-08-17 | 47 | 32 killed / 0 residual (15 unviable) | Filed as #273 by the owner, who swept it first and found it had never been swept at all: **13 of 32 viable mutants survived — a 41% miss rate — in two clusters, neither of them a hole in what the envelope DOES.** (1) Nine were the reported statistics. `pack_then_unpack_returns_the_original_bytes` asserted `bytes() > 0` and `ratio() < 1.0`, which a constant satisfies, so `Packed::bytes -> 1`, `ratio -> 0.0`, `ratio -> -1.0` and the `elapsed_secs` constants all lived: the round-trip proved the envelope works, and nothing proved the numbers describing it were the real ones. `ratio` is the one that matters, because it is what a caller decides by, and it is now compared against `output/input` computed from the file on disk. (2) Four were `Debug` impls, each stubbable to `Ok(Default::default())`. Not the two that carry a passphrase — `a_passphrase_never_appears_in_a_debug_rendering` asserts the redaction MARKER is present, not merely that the secret is absent, so a stub fails it. It was the four with no secret in them, which were therefore in no test's scope and had never been read by anything; their counterpart now asserts each rendering names the struct it claims to be and carries its fields, and that `PackBytes` shows the payload's LENGTH rather than the payload. **The re-sweep then caught the fix itself.** `elapsed_secs` had been given a property — positive and finite — and `1.0` satisfies both, so it survived on both `Packed` and `Unpacked` where the other constants died. Replaced with an INTERVAL: the call is timed from outside and the reported figure must fall at or under the wall time, which is exact on any machine and so cannot go flaky the way a hand-picked ceiling would. Worth recording that the seal path cannot make that assertion do any work — scrypt at the shipped work factor is expensive by design, so its wall time is comfortably above `1.0` and the constant sits inside the bound. It is the pack path, whose wall is smaller by orders of magnitude, that kills it. |
<!-- END GENERATED: sweep-ledger -->

## The unswept surface

Everything **not** in the ledger is unswept — assume non-falsifiable tests may
hide there until a sweep says otherwise. The whole `laterite-cli` binary
...
DuckDB-free core, the `laterite-ags4-validator` numbered-rules engine, and the
`laterite-ags4-emit` orchestrator + byte writer are now swept — **the near-term
engine queue is complete**. What remains unswept is the QA/dev tooling
(`forge`, `corpus-qa`, `perf`, `parity`, `xcheck`) and the host bindings
(`laterite-py`, `laterite-node`, `laterite-ags4-wasm`), which
[[coverage-campaign]] sequences next.
