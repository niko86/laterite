...
Grid: ✓ present · — absent · ≈ divergent (present but shape differs from siblings). Findings: 🔴 P1 · 🟠 P2 · 🟡 P3 · ⚪ by-design. Generated from [[crate-map|the crate map]]'s surfaces via `repo:tools/gen_modality.py`; the SSOT is `repo:modality.json` and the standing gate is `repo:packages/laterite/tests/test_modality_parity.py`.
