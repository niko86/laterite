...
**Read-only consumers** reach the same union without the rule engine: `repo:rust-packages/laterite-ags4-core/src/effective_dict.rs` re-exports the module (the `registry` move, repeated) and adds the adapter for core's own read codec, so a reader that depends on `laterite-ags4-core` alone can bind a file-declared group's columns. Since #815 the union is also the READ side's **identity** authority: the keychain's `group_row_ids_effective` mints a file-declared group's `_id`/`_parent_id` from its declared KEY tuple (`FileDict::key_headings`, the same `DICT_STAT` predicate Rule 10a keys off) and its declared parent — a registry group keeps its spec keys untouched, so identity never forks per delivery file, and a group declaring no KEY headings mints nothing rather than stamping every row with the empty-chain id. The strict cousin is the [[dec-custom-dict-overlay]] `--dict` path (`parse_dict`): that one ingests a *standalone* dictionary file and refuses a bad one — a bad DICT group inside a delivery file is a finding on the file, never a refusal to read it.
