...
| write | 8.9× | **13.9×** (was 14.5× pre-M6, 18.8× pre-M4) | `build_ags4(...).save()` | **queue M2 — re-priced 2026-09-01 (#848)**: the peak carries the parse hold **twice** (the caller's retained parse + the emit door's own validating parse-back). The mechanism split: the layout row (M6, #850) was sequenced first — its spike missed the floor and the owner landed it at the measured prize (−4.1% here at 265 MB; the double hold means the door's re-price starts from this slimmer lane) — and this door's ticket is minted after a re-price on the post-layout lane ([[dec-emit-streamed-verdict]]) |
