...
**#5 — still open (measurement debt).** `Sidecar::assemble` walks the file a
second time inside `mint` (over bytes it already parsed) + a third full-buffer
hash pass, but `mint`/`certify` has **no bench** — the prize is unquantified. It
is a per-*file* operation (certify-time), not per-read, so it is a colder path
than T1–T4's per-row/per-cell targets. **Bench first** (a `laterite-ags4-trust`
`mint`/`certify` bench) before any code change; the reuse must not silently change
the non-UTF-8 mint semantics.
**Exit:** the new trust bench moves ≥10%, else #5 is declined with its number too.
