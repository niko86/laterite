...
| write | 8.9× | **14.5×** (was 18.8× pre-M4) | `build_ags4(...).save()` | **queue M2 — re-priced 2026-09-01 (#848)**: the peak carries the parse hold **twice** (the caller's retained parse + the emit door's own validating parse-back). The mechanism split: the layout row (M6, #850) was sequenced first and **declined on its spike** (2026-09-01 — see the M6 queue row), so this door's re-price question returns to the owner on the standing layout ([[dec-emit-streamed-verdict]]) |
...
| M6 | the retained parse **structure** — per-row span-vec heap blocks plus source-byte fields the validator never reads | validate + read_typed (and every `ParsedFile` holder: every write door's input half, and M2's constructed verdict had it landed) | was priced by the #848 attribution as most of the retained hold beyond the buffer, sitting *at* rule 10's gate; **DECLINED 2026-09-01 (#850)** — the spike's A/B/A on the lane instrument measures the full flattening at **−6.9/−6.2% (25 MB) and −13.1/−10.7% (100 MB)** of the validate/read_typed peaks, against a 20% floor on all four gate cells (A-spread ≤ 0.05%; A-legs reproduce the committed cells within 0.25%). The criterion riders came in fine — `parse_bytes` −9…−15%, `check_parsed` unchanged — and cannot land the break on the time axis either: parse is ~a third of any composite, so the rider is ~3–5% there, the original time-queue-#4 arithmetic again | the shape was built exactly as designed (per-group arena + slim 12-byte row index, `RawLine` 24 → 16 B, per-row/per-line offsets profile-gated — the inventory held: nothing outside the leaf's own tests reads them) and the structure is real — but the requested-bytes pricing OVERstated its RSS contribution, the inverse error to the undercount the design reasoned from: the span content itself (8 B/cell) survives in the arena by construction, and the old per-group rows-Vecs' doubling headroom was largely never-touched pages RSS never paid (requested 123.2 MB vs ~68 MB content at the 100 MB rung, #848's own table). What flattening removes — the row/line slimming plus allocator block overhead — is ~13% of the operation peak at 100 MB, ~7% at 25 MB where the import floor pads the denominator. dhat prices requests; RSS pays touched pages — in both directions | **declined** — [[dec-parse-structure-layout]] flipped to `rejected` with the record on #850; spike branch local-only, nothing landed. M2's "re-price on the post-layout lane" precondition is void — see its row |
...
- **The split verdict** (grilled with the owner, 2026-09-01): the
  ...
  its own gate and the row is declined — the M6 queue row carries the
  numbers; the sequencing decision itself was sound both ways, which is
  what spike-gating is for.)
