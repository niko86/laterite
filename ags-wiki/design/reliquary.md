...
| 7 | `read_typed(attachments_dir=)` no-op arg | `laterite/ags4.py` | intentional signature-stability anchor | overtaken: the arg is GONE — last present at `300d34ca` (2026-06-24 sync), removed in the satellite era without this row hearing about it; a repo-wide grep finds zero hits (week-36 curation) | removed |
...
| 14 | Two metadata-group synthesizers: emit's `synthesize_metadata`/`collect_units`/`collect_types`/`collect_abbreviations` (UNIT/TYPE/TRAN/ABBR, shipped) vs forge's `collect_unit`/`collect_type`/`tran`/`collect_abbr` (fuzz fixtures) | `laterite-ags4-emit/src/emit.rs` + `laterite-ags4-forge/src/synth/model.rs` | `laterite_ags4_emit::catalog` — `units_used`/`types_used` over a `GroupView` trait, two real adapters (emit's `OwnedGroup`, forge's `Group`; homed in emit, not the types leaf, because a packaged emit cannot call a types module the registry does not carry yet) | **#924** — the "converge only if they drift" trigger FIRED: forge kept padded units emit trimmed, admitted the header literals, and never learned emit's PU harvest (so the dogfood corpus could not manufacture the picklist-of-units case the shipped emitter handles). Only the *collection rule* converged; emit's gap-filling and forge's whole-file minting stay separate, and TRAN/ABBR were not drifting so stay per-consumer (see [[ags4-output]]) | converged |
| 15 | `tools/coverage-combined.sh` + the `ci/combined-coverage` local branch (instrumented the maturin `.so` so pytest credited the PyO3 glue → one honest combined Rust coverage number) | local branch only (never on `master`; head `094e4c5`, reflog-recoverable ~90d) | the `coverage` job **excludes** `laterite-py` from the Rust floor instead of instrument-and-credit; its main driver (`ags5db` Python-tested Rust) was decoupled out of this tree (#177) | **chore/retire-combined-coverage** (2026-06-26) — stale (218k behind master; its `cd packages/laterite-ags5` path went with the decouple), unused on master, owner confirmed not useful | removed |
...
| `CliError` ags5-compat variants | `rust-packages/laterite-ags4-core/src/error.rs::PreVersion65` | code | removed | ags5-relics PR | `PreVersion65`/`UnknownGroup`/`Predicate`/`UnsupportedFeature`/`Sql`/`Validation` — 0 shipped construction (AGS5 strand only); `CliError` now `FileNotFound`+`Schema` |
| `Registry::extended_with()` | `rust-packages/laterite-ags4-core/src/registry.rs::fn extended_with` | code | removed | ags5-relics PR | passthrough-registration; 0 shipped callers (2 in the AGS5 strand) |
| `heading_storage_index()` | `rust-packages/laterite-ags4-core/src/registry.rs::fn heading_storage_index` | code | removed | ags5-relics PR | ags5 parent-overlap-dedup helper; 0 shipped callers (2 in the AGS5 strand) |
...
| `tests/p1-flagship.cjs` (node) | rust-packages/laterite-node/tests/p1-flagship.cjs::P1 — the native engine core | code | removed | the #866 knip-green PR (its scheduled slot: the PR that next touched the node surface's test wiring) | A pre-vitest CommonJS smoke script, sole occupant of `tests/` — and `vitest.config.ts` reads `test/`, one letter away, which is why nothing has run it and nothing noticed. Zero references repo-wide (no `package.json` script, no workflow, no doc); it arrived in a pre-flip `sync: public tree` commit and no commit has touched it since. What it asserts — parse→Arrow IPC, validate, byte-faithful emit, data→AGS4 — is covered by the `test/` suite that replaced it, so this is a superseded copy rather than lost coverage. Found by `knip` (the only genuine orphan in the 2026-08-21 three-scout pass; vulture's four `tools/` hits all had live callers in `tests/`, and web's 70 knip hits are overwhelmingly the #418 Solid primitives, built ahead of the app restyle they are for). **Confirm before deleting**: a file vitest cannot see is exactly the shape that reads as dead while being someone's manual smoke test. **Removed 2026-09-02 (#866):** the confirm-before-deleting check re-ran at removal — still zero references repo-wide, and the `test/` suite still covers every assertion it made; `npm run knip` goes green with it gone. |
...
| `group_from_arrow()` | `rust-packages/laterite-ags4-emit/src/arrow_in.rs::pub fn group_from_arrow(` | code | removed | feat/emit-arrow-door-790 (#790 PR2) | The Arrow→cell transpose's no-overrides entry; only caller was the heap_profile instrument (a `let _ =` doc-honesty reference). Superseded by `emit_ags4_from_arrow`, which streams batches straight into the formatted OwnedGroup. Published on crates.io ≤0.13.0, so "no in-repo caller" was checked, not assumed — the crate takes the break under its 0.x line. |
| `group_from_arrow_with_meta()` | `rust-packages/laterite-ags4-emit/src/arrow_in.rs::pub fn group_from_arrow_with_meta<` | code | removed | feat/emit-arrow-door-790 (#790 PR2) | Override-map variant of the transpose; hosts had all moved to `_at_edition`. Its {heading→value} alignment survives inside `owned_group_from_arrow`. |
| `group_from_arrow_with_meta_at_edition()` | `rust-packages/laterite-ags4-emit/src/arrow_in.rs::pub fn group_from_arrow_with_meta_at_edition` | code | removed | feat/emit-arrow-door-790 (#790 PR2) | The transpose entry all three hosts actually called (#695 DT precision rode its `edition`). The DT-precision behaviour moved into the streaming door unchanged — `opts.edition` drives it; the `edition: None` pre-#695 rendering it also offered is retired with it (every host passed Some). |
| `cell_value()` (public) | `rust-packages/laterite-ags4-emit/src/arrow_in.rs::pub fn cell_value` | code | removed | feat/emit-arrow-door-790 (#790 PR2) | Demoted to pub(crate), not deleted: it is still the ONE type→cell mapping both doors share — `owned_group_from_arrow` streams through it per cell. What was retired is the public export nothing outside the crate called. |
