...
| `GroupDescriptor::table()`/`view()` | `rust-packages/laterite-ags4-reference/src/union.rs::fn table` | code | spotted | — | DuckDB `g_<code>`/`v_<code>` name builders on the reference-data leaf. Verified dead 2026-07-29: the only occurrences repo-wide are the two fns and their own unit test (union.rs:401-402), and `laterite-duckdb` never references `GroupDescriptor` — it builds no `g_`/`v_` names at all. Publishing would freeze an unrelated product's naming into a dictionary crate's public API. See [[dec-rust-api-crates-io]], laterite#161 |
