...
| `feeds_ags5_req` (insight frontmatter) | ags-wiki/templates/_template-insight.md::feeds_ags5_req | doc | spotted | not yet — a deliberate insight-template pass, one page class at a time | Stranded by #500. The field fed the AGS5 requirement register, which that change retired; it is carried by every insight page and is EMPTY on all of them, so it has never pointed at anything. Not swept with #500 because it is a different unit of work: the class was two files and four enumerations, this is a frontmatter key on 31 pages plus its template, and lint reads required keys off that template — dropping it there is safe (only MISSING keys are flagged) but leaves 31 pages carrying a key nothing documents. `feeds_strategy`, its sibling, IS live and stays. |
