...
supersedes: []
from_gap: []
related: [dec-laterite-types-leaf, crate-map, crate-dependency-graph, ags4-corpus-qa, data-single-source-audit, laterite-ags4-wasm, dec-ags4-merge-semantics]
sources: []
---
...
Two client tools independently "anonymise" AGS4 files. `ags4-corpus-qa`'s
`censor` subcommand (pre-#581: `ags4-corpus-qa/src/censor.rs` in full) scrubs
...
copy of `ags_dictionary.json`). `ags4-corpus-qa` now depends ON the leaf and
keeps only its crawler/manifest/rayon/report wrapper
(`repo:rust-packages/ags4-corpus-qa/src/censor.rs`). Added to the workspace
...
real-world structure (feeding O-30..O-34, per [[ags4-corpus-qa]]). The
...
- `ags4-corpus-qa` no longer owns any scrub logic of its own; changing an
...
- `crate-dependency-graph.md` (generated by `repo:tools/gen_crate_graph.py`)
  ...
  `parse`/`reference`/`types` → `censor` → `ags4-corpus-qa` **and**
...
```mermaid
graph TD
  ssot[sensitive_headings.json SSOT] --> corpus["ags4-corpus-qa/src/censor.rs<br/>private tokenizer + 5 actions"]
...
graph TD
  ...
  censor --> corpusqa[ags4-corpus-qa]
...
[[ags4-corpus-qa]] · [[data-single-source-audit]] · [[laterite-ags4-wasm]] ·
