...
## Definition
> [!quote] AGS4 TYPE `U` — `spec:AGS4-4.2-2025.pdf §3.3`
> Value with a variable format — for fields with differing accuracy (e.g. AGS 4.1/4.2 ELRG_RVAL).

## Canonical mapping
Maps to `CanonicalType.string` — cite `repo:packages/laterite/python/laterite/ags_types.py::canonical_type`.

## Validation (Rule 8)
> [!quote] Variable-format value — Rule 8 is lenient (no fixed shape to enforce); used for differing-accuracy fields (e.g. ELRG_RVAL).
