---
type: observation
title: O-07 — Rule 19b_1's field-length limit isn't in the prose (but the dict obeys it)
status: drafted
tags: [observation]
observation_id: O-07
obs_tag: SPEC
phase: V3
upstream_reportable: true
reported: ""
cross_obs: [O-06, O-09]
rules_touched: [7, 19]
ags_editions: [4.0.3, 4.0.4, 4.1, 4.1.1, 4.2]
repo_refs:
  anchor: "repo:OBSERVATIONS.md#o-7"
related: [parity-model, upstream-reporting, O-06]
sources: []
---
# O-07 — Rule 19b_1's field-length limit isn't in the prose (but the dict obeys it)

> Source-of-truth: `repo:observations.json` (record O-7), rendered at `repo:OBSERVATIONS.md#o-7`.
> This page links + cross-references only — it never copies the 5 fields.

## Summary
> [!spec] **SPEC.** Rule 19b_1's ≤4-char field-part limit is in no prose but the dictionary obeys it (0/4199 deviate). Same prose-vs-de-facto pattern as O-6.

Source-of-truth (never pasted): `repo:observations.json`, record O-7 — rendered at `repo:OBSERVATIONS.md#o-7`.

## Rules affected
[[rule-19-group-name-format]]

## Cross-observation graph

```mermaid
graph LR
  O_07["O-07"]
  O_07 --- O_06["O-06"]
```

## Upstream status
> [!note] Upstream-reportable: [SPEC] — state the field-part ≤4 convention explicitly.

_Not yet marked Reported in OBSERVATIONS.md._

## Related
[[parity-model]] · [[upstream-reporting]] · [[O-06]]
