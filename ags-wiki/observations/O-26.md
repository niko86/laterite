---
type: observation
title: O-26 — 19b_2/19b_3 re-report headings already flagged by 19b_1/9
status: drafted
tags: [observation]
observation_id: O-26
obs_tag: NOTE
phase: V8
upstream_reportable: false
reported: ""
cross_obs: [O-13]
rules_touched: [9, 19]
ags_editions: [4.0.3, 4.0.4, 4.1, 4.1.1, 4.2]
repo_refs:
  anchor: "repo:OBSERVATIONS.md#o-26"
related: [parity-model, upstream-reporting, O-13]
sources: []
---
# O-26 — 19b_2/19b_3 re-report headings already flagged by 19b_1/9

> Source-of-truth: `repo:observations.json` (record O-26), rendered at `repo:OBSERVATIONS.md#o-26`.
> This page links + cross-references only — it never copies the 5 fields.

## Summary
> [!note] **NOTE.** python rule_19b_2/19b_3 re-report a malformed heading already flagged by 19b_1 + Rule 9 — up to 3× under 19b plus 1× under 9. We gate 19b_2/3 to the genuine borrowed-heading case only; presence parity holds.

Source-of-truth (never pasted): `repo:observations.json`, record O-26 — rendered at `repo:OBSERVATIONS.md#o-26`.

## Rules affected
[[rule-19-group-name-format]]

## Cross-observation graph

```mermaid
graph LR
  O_26["O-26"]
  O_26 --- O_13["O-13"]
```

## Upstream status
> [!note] Upstream-reportable: [NOTE] — python could gate 19b_2/3 on 'prefix≠group & has underscore'.

_Not yet marked Reported in OBSERVATIONS.md._

## Related
[[parity-model]] · [[upstream-reporting]] · [[O-13]]
