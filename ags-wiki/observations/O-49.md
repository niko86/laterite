...
> Source-of-truth is `repo:OBSERVATIONS.md#o-49`.
...
> `format_ndp`/`format_nsci` fed a bare `usize` straight into `{:.n$}`. #610
> clamps every count-driven formatter (6 sites, `laterite-types` +
> `laterite-excel`) to `MAX_NUMERIC_COUNT = 30` first — a bounded-output
...
> ratified as [[O-50]] (#611) — unbounded OUTPUT here, unbounded INPUT into
> a bounded `i64` store there.

Source-of-truth (never pasted): `repo:OBSERVATIONS.md#o-49`.

## Rules affected
[[rule-08-typed-values]] — the grammar check is the validator's own path to
the formatter; the fixes engine and the XLSX writer (`laterite-excel`) are
