---
type: observation
title: "O-09 — Rule 7's \"no duplicate headings\" is inferred, not in the prose"
status: drafted
tags: [observation]
observation_id: O-09
obs_tag: NOTE
phase: V4
upstream_reportable: false
reported: ""
cross_obs: [O-06, O-07]
rules_touched: [4, 7]
ags_editions: [4.0.3, 4.0.4, 4.1, 4.1.1, 4.2]
repo_refs:
  anchor: "repo:OBSERVATIONS.md#o-9"
related: [parity-model, upstream-reporting, O-06, O-07]
sources: []
---
# O-09 — Rule 7's "no duplicate headings" is inferred, not in the prose

> Source-of-truth: `repo:observations.json` (record O-9), rendered at `repo:OBSERVATIONS.md#o-9`.
> This page links + cross-references only — it never copies the 5 fields.

## Summary
> [!note] **NOTE.** Rule 7's 'no duplicate headings' is inferred (implied by Rules 4/7/8 together), not literal prose. We keep the check.

Source-of-truth (never pasted): `repo:observations.json`, record O-9 — rendered at `repo:OBSERVATIONS.md#o-9`.

## Rules affected
[[rule-07-heading-order]]

## Cross-observation graph

```mermaid
graph LR
  O_09["O-09"]
  O_09 --- O_06["O-06"]
  O_09 --- O_07["O-07"]
```

## Upstream status
> [!note] Upstream-reportable: [NOTE] — AGS-DFWG could make no-duplicate-HEADING explicit.

_Not yet marked Reported in OBSERVATIONS.md._

## Related
[[parity-model]] · [[upstream-reporting]] · [[O-06]] · [[O-07]]
