---
type: observation
title: "O-21 — Rule 10c's parentless-group list is hardcoded, not dict-derived"
status: drafted
tags: [observation]
observation_id: O-21
obs_tag: SPEC
phase: V7
upstream_reportable: true
reported: ""
cross_obs: [O-55]
rules_touched: [10]
ags_editions: [4.0.3, 4.0.4, 4.1, 4.1.1, 4.2]
repo_refs:
  anchor: "repo:OBSERVATIONS.md#o-21"
related: [parity-model, upstream-reporting, non-hierarchy-ten-vs-parentless-list]
sources: []
---
# O-21 — Rule 10c's parentless-group list is hardcoded, not dict-derived

> Source-of-truth: `repo:observations.json` (record O-21), rendered at `repo:OBSERVATIONS.md#o-21`.
> This page links + cross-references only — it never copies the 5 fields.

## Summary
> [!spec] **SPEC.** Rule 10c's parentless set is HARDCODED (PROJ,TRAN,ABBR,DICT,UNIT,TYPE,LOCA,FILE,LBSG,PREM,STND — 11, incl. LOCA) not DICT_PGRP-derived: LOCA's DICT_PGRP is PROJ yet LOCA rows carry no PROJ key, so a dict-derived check would emit bogus orphans. Maintenance hazard. (Spec §3.1 lists only 10 non-hierarchy groups — see [[non-hierarchy-ten-vs-parentless-list]].)

Source-of-truth (never pasted): `repo:observations.json`, record O-21 — rendered at `repo:OBSERVATIONS.md#o-21`.

## Rules affected
[[rule-10a-key-uniqueness]]

## Cross-observation graph

```mermaid
graph LR
  O_21["O-21"]
```

## Upstream status
> [!note] Upstream-reportable: [SPEC] — make parentless/implicit-link a dictionary property so Rule 10c is data-driven.

_Not yet marked Reported in OBSERVATIONS.md._

## Related
[[parity-model]] · [[upstream-reporting]] · [[O-55]] — the other half of the
parentage path: whether `DICT_PGRP` must be populated at all is answered three
ways by the spec, and this rule is where a blank one surfaces.
