---
type: observation
title: O-06 — Rule 19 spec prose is looser than the AGS dictionary
status: drafted
tags: [observation]
observation_id: O-06
obs_tag: SPEC
phase: V3
upstream_reportable: true
reported: ""
cross_obs: [O-07, O-09]
rules_touched: [19]
ags_editions: [4.0.3, 4.0.4, 4.1, 4.1.1, 4.2]
repo_refs:
  anchor: "repo:OBSERVATIONS.md#o-6"
related: [parity-model, upstream-reporting]
sources: []
---
# O-06 — Rule 19 spec prose is looser than the AGS dictionary

> Source-of-truth: `repo:observations.json` (record O-6), rendered at `repo:OBSERVATIONS.md#o-6`.
> This page links + cross-references only — it never copies the 5 fields.

## Summary
> [!spec] **SPEC.** Rule 19 prose permits ≤4 'letters AND numbers'; python (and we) enforce the de-facto 'exactly 4 uppercase letters' the dictionary universally follows (0/319 deviate). Prose ⇄ dictionary disagree.

Source-of-truth (never pasted): `repo:observations.json`, record O-6 — rendered at `repo:OBSERVATIONS.md#o-6`.

## Rules affected
[[rule-19-group-name-format]]

## Cross-observation graph

```mermaid
graph LR
  O_06["O-06"]
```

## Upstream status
> [!note] Upstream-reportable: [SPEC] — AGS-DFWG should tighten Rule 19 wording to 'exactly 4 uppercase letters'.

_Not yet marked Reported in OBSERVATIONS.md._

## Related
[[parity-model]] · [[upstream-reporting]]
