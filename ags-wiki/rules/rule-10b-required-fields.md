---
type: rule
title: Rule 10b — required fields
status: drafted
tags: [rule]
rule_number: 10
rule_sub: b
...
repo_refs:
  ...
  fixtures: ""
  regression: ""
  spec: "spec:AGS4-4.2-2025.pdf §4.1.1 Rule 10b"
related: [rule-families, traceability-chain, parity-model]
sources: [spec-4.2]
---
# Rule 10b — required fields

## Statement
> [!quote] AGS4 Rule 10b — `spec:AGS4-4.2-2025.pdf §4.1.1 Rule 10b`
...
> [!quote] Implemented in `repo:rust-packages/laterite-ags4-validator/src/rules/relational.rs::rule_10b`

REQUIRED fields present + non-null in every DATA row. Rule 12 is wholly subsumed by 10b (O-15).
...
```mermaid
flowchart LR
  R["Rule 10b"] --> I["relational.rs"] --> F["0 fixture(s)"] --> T["0 test(s)"] --> O["O-N (linked at Ingest)"]
```
