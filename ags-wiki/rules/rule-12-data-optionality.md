---
type: rule
title: Rule 12 — data optionality
status: drafted
tags: [rule]
rule_number: 12
rule_sub: ""
...
repo_refs:
  ...
  fixtures: ""
  regression: ""
  spec: "spec:AGS4-4.2-2025.pdf §4.1.1 Rule 12"
related: [rule-families, traceability-chain, parity-model]
sources: [spec-4.2]
---
# Rule 12 — data optionality

## Statement
> [!quote] AGS4 Rule 12 — `spec:AGS4-4.2-2025.pdf §4.1.1 Rule 12`
> Data does not have to be included against each HEADING unless REQUIRED (Rule 10b). The data FIELD can be null; a null entry is defined as "" (two quotes together).
...
No-op — null-allowed semantics wholly covered by Rule 10b (O-15).
...
```mermaid
flowchart LR
  R["Rule 12"] --> I["groups.rs"] --> F["0 fixture(s)"] --> T["0 test(s)"] --> O["O-N (linked at Ingest)"]
```
