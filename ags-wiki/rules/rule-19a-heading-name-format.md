---
type: rule
title: Rule 19a — heading name format
status: drafted
tags: [rule]
rule_number: 19
rule_sub: a
...
repo_refs:
  ...
  fixtures: "repo:rust-packages/laterite-ags4-validator/tests/fixtures/rule19a_bad_heading.ags"
  regression: "repo:rust-packages/laterite-ags4-validator/tests/regression.rs::rule19a_bad_heading_charset_flagged"
  spec: "spec:AGS4-4.2-2025.pdf §4.1.1 Rule 19a"
related: [rule-families, traceability-chain, parity-model]
sources: [spec-4.2]
---
# Rule 19a — heading name format

## Statement
> [!quote] AGS4 Rule 19a — `spec:AGS4-4.2-2025.pdf §4.1.1 Rule 19a`
...
> [!quote] Implemented in `repo:rust-packages/laterite-ags4-validator/src/rules/naming.rs::rule_19a`

Follows prose: ≤9 chars, [A-Z0-9_] only.
...
```mermaid
flowchart LR
  R["Rule 19a"] --> I["naming.rs"] --> F["1 fixture(s)"] --> T["1 test(s)"] --> O["O-N (linked at Ingest)"]
```

- Fixtures: `rule19a_bad_heading.ags`
- Regression: `rule19a_bad_heading_charset_flagged`
