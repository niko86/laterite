---
type: rule
title: Rule 2b — header rows order
status: drafted
tags: [rule]
rule_number: 2
rule_sub: b
...
repo_refs:
  ...
  fixtures: "repo:rust-packages/laterite-ags4-validator/tests/fixtures/rule2b_unit_missing.ags"
  regression: "repo:rust-packages/laterite-ags4-validator/tests/regression.rs::rule2b_missing_unit_flagged"
  spec: "spec:AGS4-4.2-2025.pdf §4.1.1 Rule 2b"
related: [rule-families, traceability-chain, parity-model]
sources: [spec-4.2]
---
# Rule 2b — header rows order

## Statement
> [!quote] AGS4 Rule 02b — `spec:AGS4-4.2-2025.pdf §4.1.1 Rule 02b`
> The GROUP HEADER rows fully define the data presented within the DATA rows for that group (Rule 8). As a minimum, the GROUP HEADER rows comprise GROUP, HEADING, UNIT and TYPE rows presented in that order.
...
GROUP HEADER = GROUP,HEADING,UNIT,TYPE in order; a missing HEADING row is attributed to Rule 4 not 2b, matching python (O-4).
...
```mermaid
flowchart LR
  R["Rule 2b"] --> I["structure.rs"] --> F["1 fixture(s)"] --> T["1 test(s)"] --> O["O-N (linked at Ingest)"]
```

- Fixtures: `rule2b_unit_missing.ags`
- Regression: `rule2b_missing_unit_flagged`
