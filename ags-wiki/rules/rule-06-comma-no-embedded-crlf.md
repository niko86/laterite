---
type: rule
title: Rule 6 — comma no embedded crlf
status: drafted
tags: [rule]
rule_number: 6
rule_sub: ""
...
repo_refs:
  ...
  fixtures: ""
  regression: ""
  spec: "spec:AGS4-4.2-2025.pdf §4.1.1 Rule 6"
related: [rule-families, traceability-chain, parity-model]
sources: [spec-4.2]
---
# Rule 6 — comma no embedded crlf

## Statement
> [!quote] AGS4 Rule 06 — `spec:AGS4-4.2-2025.pdf §4.1.1 Rule 06`
> The DATA DESCRIPTORS, GROUP names, data field HEADINGs, data field UNITs, data field TYPEs, and data VARIABLEs in each line of the data file shall be separated by a comma (,). No carriage returns (ASCII character 13) or line feeds (ASCII character 10) are allowed in or between data VARIABLEs within a DATA row.
...
> [!quote] Implemented in `repo:rust-packages/laterite-ags4-validator/src/rules/line_format.rs::rule_6`

Comma separator; embedded CR/LF check IMPLEMENTED here — python-ags4's rule_6 is a complete no-op (O-2), so Rust uniquely fires Rule 6.
...
```mermaid
flowchart LR
  R["Rule 6"] --> I["line_format.rs"] --> F["0 fixture(s)"] --> T["0 test(s)"] --> O["O-N (linked at Ingest)"]
```
