...
repo_refs:
  ...
  fixtures: "repo:rust-packages/laterite-ags4-validator/tests/fixtures/rule13_no_proj.ags"
  regression: "repo:rust-packages/laterite-ags4-validator/tests/regression.rs::rule13_missing_proj_flagged"
  spec: "spec:AGS4-4.2-2025.pdf §4.1.1 Rule 13"
...
## Statement
> [!quote] AGS4 Rule 13 — `spec:AGS4-4.2-2025.pdf §4.1.1 Rule 13`
...
PROJ present, exactly one DATA row, REQUIRED filled. Zero-row PROJ also Rule 2 (O-16).
...
```mermaid
flowchart LR
  R["Rule 13"] --> I["groups.rs"] --> F["1 fixture(s)"] --> T["1 test(s)"] --> O["O-N (linked at Ingest)"]
```

- Fixtures: `rule13_no_proj.ags`
- Regression: `rule13_missing_proj_flagged`
