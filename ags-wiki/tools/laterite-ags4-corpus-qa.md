...
> [!note] **Not published** — `laterite-ags4-corpus-qa` is a workspace crate, internal to this repo, at v0.11.0 (inherited from the workspace).
...
- **`baseline`** — freeze (`--out`) or drift-check (`--check`) a
  ...
  gate <!-- cadence: compliance --> — that tree runs the compliance harness,
  not this one — pinning the
