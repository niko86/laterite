...
- (retired 2026-09-04) `rust-packages/laterite/Cargo.toml` — the facade carried
  its own 0.1.x while it was short of parity. dec-facade-parity phase 8 jumped
  it onto PRODUCT, and it IS gated now, below, together with the parity floor
...
ENGINE_CRATES = {
    ...
    for name in sorted(
        ...
        and d.name != "laterite"  # the facade is PRODUCT (phase 8), asserted below
    )
}
...
def test_the_facade_is_on_the_product_line() -> None:
    """`rust-packages/laterite` carries the product number — the phase-8 jump.

    Until 2026-09-04 this test held the OPPOSITE: the facade on its own 0.1.x,
    an explicit exemption while it was short of parity with the Python and Node
    surfaces. dec-facade-parity phase 8 took the one-way door — 0.1.x straight
    onto the product number — so now the assertions are the parity claim itself:
    the number matches, the product bump stamps it, and the version stays a
    literal (inheriting the workspace line would silently move it onto the
    unpublished members' clock).
    """
    facade = _toml("rust-packages/laterite/Cargo.toml")["package"].get("version")
    assert isinstance(facade, str), (
        "the facade must carry a literal version, not inherit one — the "
        "workspace number belongs to the unpublished members, not the product"
    )
    assert facade == PRODUCT, (
        f"the facade is {facade!r}, not the product {PRODUCT!r} — since the "
        "phase-8 jump `cargo add laterite` and `pip install laterite` are the "
        "same release, and a facade left behind breaks that claim"
    )
    product_files = {
        f["filename"] for f in _toml("pyproject.toml")["tool"]["bumpversion"]["files"]
    }
    assert "rust-packages/laterite/Cargo.toml" in product_files, (
        "the facade carries the product version but nothing stamps it — the "
        "next `bump-version.sh product` would leave it behind"
    )


def test_the_facade_floor_holds() -> None:
    """The parity the product number claims, asserted against the register.

    Decision 7 of dec-facade-parity (as revised 2026-09-04): parity means every
    below-floor row is SETTLED — `to add == 0 and UNDECIDED == 0`, with a row
    allowed to sit below only under a recorded `facade_verdict: "by-design"`
    whose note names a true, published door. The floor itself stays mechanical
    (the intersection of what Python and Node offer, computed by
    `gen_modality.py`, never stored), so this cannot be satisfied by editing a
    stored baseline — only by building a door, or by recording a verdict where
    the register's census prints it on every run.

    Reuses the generator's own `_facade_debt` rather than restating the floor
    ...
    for cap in register["capabilities"]:
        d_in, d_out = mod._facade_debt(cap["cells"])
        if not (d_in or d_out):
            continue
        cell = next((c for c in cap["cells"] if c["surface"] == "rust"), None)
        verdict = (cell or {}).get("facade_verdict", "undecided")
        if verdict != "by-design":
            unsettled.append(f"{cap['capability']} ({verdict})")
    assert not unsettled, (
        f"below-floor row(s) with no by-design verdict: {unsettled} — the facade "
        f"carries the product number {PRODUCT!r} on the strength of parity, so "
        "either build the door or record the owner's verdict in modality.json"
    )
