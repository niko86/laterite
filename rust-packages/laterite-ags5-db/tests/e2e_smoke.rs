...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR points at `laterite-ags5-db/`; the workspace root is one up.
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("laterite-ags5-db has a parent")
        .to_path_buf()
}

fn bin() -> Command {
    Command::cargo_bin("lat-db").expect("lat-db binary built")
}
...
fn version_includes_crate_version() {
    bin()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("lat-db"));
}
...
"#;
    ...
    // not a flat dump — so `lat-check --check-files` on the emitted
