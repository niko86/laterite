...
//! the `laterite-ags4-validator` skill). This binary trusts its input and only
...
pub struct Cmd {
    ...
    /// `FILE/<FILE_FSET>/<FILE_NAME>` (spec-check with `lat-check
    /// --check-files`).
    pub ags4: PathBuf,
...
pub fn run(args: Cmd, ctx: Ctx) -> anyhow::Result<()> {
    ...
    let t0 = Instant::now();
    let stats = laterite_ags5_db::convert::db_to_ags4(&args.db, &args.ags4)?;
    ...
    let attach_stats = laterite_ags5_db::attachments::unspool_attachments(&args.db, &out_dir)?;
    ...
    if args.validate {
        use laterite_ags4_validator::{CheckOptions, check_file, findings};
