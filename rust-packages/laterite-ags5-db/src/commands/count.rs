//! `lat-db count <db> <group> [--where ...]` — COUNT(*) on a group's view.
//!
//! Mirrors Python `_cmd_count`. Returns a scalar integer. The data work
//! (predicate validation + parameterised SELECT) lives in
//! `laterite_ags5_db::query::count`; this is the clap + scalar-render shim.
...
pub fn run(args: Cmd, ctx: Ctx) -> anyhow::Result<()> {
    let n = laterite_ags5_db::query::count(&args.db, &args.group, &args.r#where)?;
