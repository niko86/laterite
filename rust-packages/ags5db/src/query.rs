...
// for `ags5_xml.validation.validate_ags5db` (Stage F2a-2).
//
// Two checks per row:
...
use ags5_core::ags_types::{CanonicalType, canonical_type, parse_value};
...
pub fn validate_db(
    ...
    for g in reg.iter() {
        let pa_headings: Vec<&ags5_core::registry::Heading> = g
            .headings
            .iter()
            .filter(|h| h.ags_type.eq_ignore_ascii_case("PA"))
            .collect();
        let dt_headings: Vec<&ags5_core::registry::Heading> = g
            .headings
            .iter()
            .filter(|h| {
