//! `ags5db peek <db> <group>` — safe row browser over `v_<group>`.
//!
//! Mirrors Python `_cmd_peek`. The query (field/where/limit/offset +
//! optional null-column drop) lives in `ags5db::query::peek`; this shim
...
use ags5db::db::type_labels_for;
use ags5db::query::{all_null_columns, drop_columns, peek};
