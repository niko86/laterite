//! `ags5db sql <db> <statement>` — raw DuckDB SELECT (read-only).
//!
...
//! `ags5db::query::sql`; this shim adds clap + the auto-limit hint + render.
...
pub fn run(args: Cmd, ctx: Ctx) -> anyhow::Result<()> {
    ...
    let rows = ags5db::query::sql(&args.db, &args.sql, args.limit, args.explain)?;
