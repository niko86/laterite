...
use ags5_core::ags4_codec::read_ags4;
use ags5_core::ags4_writer::{EmitGroup, write_ags4};
use ags5_core::ddl::build_ddl;
use ags5_core::error::CliError;
use ags5_core::registry::{GroupDescriptor, Heading, Registry, inherited_key_names, registry};
...
fn build_passthrough_descriptors(
    reg: &Registry,
    parsed: &ags5_core::ags4_codec::ParsedAgs4,
) -> Vec<GroupDescriptor> {
...
fn preload_codec_state(conn: &Connection, reg: &Registry) -> Result<CodecCtx, CliError> {
    use crate::db::value_to_json;
    use ags5_core::ags_types::ags4_str;
...
/// Matches Python `_resolve_parent_uuid`.
        ...
        if let Some((gc, hn)) = rl_hits.first() {
            return Err(CliError::UnsupportedFeature(format!(
                "AGS4 Record Link headings (RL type) at {}.{}{}: \
                 ...
                 `ags4-validator` skill for spec context.",
                gc,
                hn,
                if rl_hits.len() > 1 {
