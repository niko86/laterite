//! `ags5db` — Rust CLI for AGS5 `.ags5db` files.
//!
...
// The CLI-dep-free logic lives in the `ags5db` library (src/lib.rs);
...
// indicatif rendering). Logic is reached via `ags5db::<module>`.
mod commands;
mod output;
...
fn main() {
    ...
    // include_str!. Inlined (not via ags-cliutil) because ags5db is
