...
the Python `ags5db` CLI.

## Usage

    ags5db <command> [args] [global flags]
    ags5db --readme         # this document
    ags5db --help           # full command list
    ags5db <command> --help # flags for one command
...
## Examples
    ...
    ags5db info site.ags5db
    ags5db peek site.ags5db LOCA --where "LOCA_GL > 50" -o csv
    ags5db sql  site.ags5db "SELECT LOCA_ID FROM v_LOCA LIMIT 5" --json
    ags5db ags4-to-db delivery.ags site.ags5db
    ags5db db-to-ags4 site.ags5db out.ags --validate
    ags5db diff old.ags5db new.ags5db        # exit 1 if they differ
...
that tree with `ags4-check out.ags --check-files` (the on-disk Rule 20
