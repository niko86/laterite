//! AGS4 emitter — counterpart to `ags4_codec.rs`'s reader.
//!
...
pub fn write_ags4<W: Write>(out: &mut W, groups: &[EmitGroup<'_>]) -> Result<(), CliError> {
...
fn write_row<W: Write>(out: &mut W, cells: &[&str]) -> Result<(), CliError> {
...
mod tests {
    ...
    fn round_trip_via_reader() {
        // Build a tiny fixture, write it, re-parse via the AGS4 reader,
        // confirm headings / rows survive.
