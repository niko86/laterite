//! `ags4-check` — CLI wrapper around the validator library.
//!
...
//! JSON. Those shared primitives now live in the `ags-cliutil` crate
//! findings-specific JSON/NDJSON/table *shaping* stays here. (The lib
//! still links none of it — `ags-cliutil` is bin-only.)
//!
//! ```text
//! ags4-check <file.ags> [--dict-version 4.1|4.2] [--dict <path>]
...
use ags_cliutil::{Spinner, colour_enabled, styled_table, write_atomic, write_json_pretty};
use ags4_validator::{CheckOptions, DictVersion, Findings, ValidatorError, check_file, findings};
...
const USAGE: &str = "\
usage: ags4-check <file.ags> [options]
...
/// the shared `ags-cliutil` `UTF8_FULL` grid (bold-cyan header,
...
/// CLI's JSON looks identical. `ags4-check` has no `--no-color` flag,
...
mod tests {
    use super::*;
    use ags4_validator::findings::add;
