...
fn main() {
    ...
    eprintln!("dhat: fixture {path} ({} bytes), stage={stage}", bytes.len());

    match stage {
        // Profile ONLY the parse (its allocations, attributed to parse_bytes).
        "parse" => {
            let _profiler = dhat::Profiler::new_heap();
            let pf =
                laterite_ags4_parse::parse_bytes(&bytes, encoding_rs::UTF_8).expect("parses");
            black_box(&pf);
        }
        // Profile ONLY the typed build: parse first (unprofiled), then time the
        // build over every group's REAL cells — the read hotspot the wheel pays.
        _ => {
            let pf =
                laterite_ags4_parse::parse_bytes(&bytes, encoding_rs::UTF_8).expect("parses");
            let _profiler = dhat::Profiler::new_heap();
