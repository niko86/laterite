...
/// Write `bytes` to `dest` via the shared staged temp-file + rename
/// (`hostopts::staged_write`, #923) — atomic on one filesystem, so `dest`
/// never holds a partial write. Shared by the build doors' file forms; what
/// each door lets REACH this write is the doors' difference, not the write's.
fn staged_write(dest: &Path, bytes: &[u8]) -> Result<(), Error> {
    laterite_ags4_emit::hostopts::staged_write(dest, bytes)
        .map_err(|e| Error::new(ErrorKind::Io, e.message))
}
