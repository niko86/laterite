...
- **Our decision**: data-level always + on-disk opt-in; the corpus-qa dogfood enables it, so the two agree directly there — and the `parity.rs` O-27 reconcile arm stays for harnesses that run with it OFF (the cross-surface compliance matrix: the duckdb surface has no filesystem to stat), where python's always-on check fires Rule 20 and Rust stays silent. `db-to-ags4` reconstructs the `FILE/<fset>/<name>` sidecar tree from stored blobs so an exported delivery passes `--check-files`.
