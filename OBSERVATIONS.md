...
### O-28 [VARIANCE] External `--dict` custom-dictionary override — deferred at V8, implemented in #568
...
- **python-ags4** (`check.py::rule_10c`): permissive — rows whose parent KEY columns are *all* empty are skipped (treated as intentionally standalone). Fixture `Standalone_SAMP_IDs.ags` (lab-control SAMP rows with no LOCA_ID) is asserted clean.
...
- **Assessment**: a clean-room structural check for a malformed dictionary the spec is silent on and python-ags4 ignores. WARNING (shown by default since #203, opt out with `--no-warnings`), not Error — the file breaks no rule, so the default verdict + the compat path are untouched. WARNING over FYI (cf. O-43): a malformed DICT is genuinely broken, not merely spec-legal-but-unusual.
...
- **Assessment**: a deliberate laterite-STRICTER categorisation (cf. O-43/O-44) — an unrecognised edition can yield a verdict against the wrong dictionary, so it belongs in the default-visible WARNING tier. WARNING, not Error: the file breaks no rule, and error-parity is untouched (the parity gate compares only `AGS Format Rule N` keys).
...
- **Our decision** (#610): clamp the count to `MAX_NUMERIC_COUNT` (30) at all six sites (laterite-types nDP/nSF/nSCI + laterite-excel Dp/Sci/format_sf) before it reaches a format width. A SEPARATE 0DP-integer truncation — laterite's `f as i64` SATURATES at ±i64::MAX where python-ags4's `f"{float(s):.0f}"` keeps full precision (e.g. 1E30 → 9223372036854775807 vs 1000000000000000019884624838656; 99999999999999999999 → 9223372036854775807 vs 100000000000000000000) — is a real value-divergence in the OTHER direction (we lose precision, python doesn't) hardened in #611 (see O-50), not this PR.

### O-50 [VARIANCE] 0DP integer CONVERSION: laterite range-guards an out-of-i64 value to Null (#611, was a fabricated i64::MAX); both validators already flag it, and python-ags4's numeric conversion keeps full precision
...
- **Us** (`laterite_types::parse_ags_integer`, the #611 single source for `parse_value` + `ags4_str` + laterite-py's PyO3 wrapper): the Integer arm used a saturating `f as i64`, so "1E30" became a FABRICATED 9223372036854775807 — a wrong number that was never in the file. #611 replaced that with a range guard: out-of-i64 → None (Null / Python `None`); in-range is untouched ("5.0"→5, "5.7"→5, "1E-30"→0), so `_content_hash` for all valid data is byte-identical. Finishes the #531 dedup (which single-sourced date/time/bool but left this Integer arm copied three ways).
...
- **Upstream-reportable**: **[NO]** — python-ags4's numeric conversion is not defective here; it preserves precision, and WE were the lossy side (silent saturation). #611 fixes it on our side. Recorded for our own decision trail, and as the sibling of O-49 (the Class B count-DoS, where python-ags4 IS the reportable side).
- **Our decision** (#611): range-guard `parse_ags_integer` so an out-of-i64 0DP value converts to Null, not a saturated integer (the "reject overflow" option). Single-sourced across the leaf's `parse_value`/`ags4_str` and laterite-py so the typed-read object and the hash canonicalisation cannot drift (the #503 RL lesson). See O-49 for the sibling numeric-TYPE-count DoS.
