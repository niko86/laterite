...
- **Plan**: listed an `lat validate --dict <path>` runtime override.
- **Reality**: [`Dictionary`] is `'static` phf-backed (zero-startup, compiled-in). A runtime-parsed dictionary needs an owned variant and a non-`'static` lifetime threaded through `DictEntry` / `GroupMeta` and every rule module — a broad, regression-prone change for a power-user feature that `db-to-ags4 --validate` (bundled 4.2) doesn't need.
- **Assessment**: deferring the risky refactor at the final phase is a conscious scope decision, not a silent skip — the flag is plumbed and returns a clear `BadDict` error (exit 5) pointing at `--dict-version`.
- **Upstream-reportable**: no.
- **Our decision**: keep bundled editions only. If a real consumer needs custom dictionaries, the clean implementation is a `Cow` / enum-backed `Dictionary` (bundled vs parsed) — a focused follow-up, not an end-of-project bolt-on.
