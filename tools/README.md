...
| Script | Purpose |
|---|---|
| `build-rust.sh` / `build-rust.ps1` | Build the Rust `lat-check` + `ags5db` binaries into `dist/`. |
...
| `parity-coverage.sh` | Clone python-ags4 1.2.0 + run its suite through `laterite.compat` with coverage — reports the 122/131 parity AND how much of `laterite.compat` it exercises. Verify the README's parity claim yourself. |
