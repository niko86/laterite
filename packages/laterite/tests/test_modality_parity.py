...
#
# It reflects from `tools/release/public-api/laterite.txt` rather than by parsing
# `src/`, because that snapshot is itself machine-generated by `cargo public-api`
# and held against the crate by its own CI gate. Reading it here chains onto a
# guarantee that already exists instead of adding a second, weaker parser — the
# register's own reflect-don't-hand-list discipline.


PUBLIC_API = REPO / "tools" / "release" / "public-api" / "laterite.txt"
