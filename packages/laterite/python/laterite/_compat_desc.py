...
_TABLE: _Rules = [
    ...
    (
        "AGS Format Rule 7",
        re.compile(
            r'^Headings out of order from "(?P<head>[^"]+)"\. '
    ...
    (
        "AGS Format Rule 8",
        re.compile(
            r'^Value "(?P<val>[^"]*)" in (?P<head>[A-Z0-9]{4}_[A-Z0-9]+) '
    ...
    (
        "AGS Format Rule 9",
        re.compile(
            r'^Heading "(?P<head>[A-Z0-9]{4}_[A-Z0-9]+)" is not in the '
    ...
    (
        "AGS Format Rule 10c",
        re.compile(
            r"^No parent entry in (?P<grp>[A-Z]{4}) for KEY combination: "
            r"(?P<keys>.*)$"
        ),
    ...
    (
        "AGS Format Rule 10c",
        re.compile(
            r"^No KEY fields are defined in the parent group "
            r"\((?P<grp>[A-Z]{4})\)\.?$"
        ),
    ...
    (
        "AGS Format Rule 10c",
        re.compile(
            r"^(?P<fields>[A-Z0-9_,\s]+) defined as KEY field\(s\) in the "
    ...
    (
        "AGS Format Rule 11c",
        re.compile(
            r'^Invalid Record Link "(?P<link>[^"]+)": "@" must separate '
    ...
    (
        "AGS Format Rule 15",
        re.compile(
            r'^Unit "(?P<u>[^"]+)" \(first used in the UNIT row of '
    ...
    (
        "AGS Format Rule 15",
        re.compile(
            r'^Unit "(?P<u>[^"]+)" \(first used in column '
    ...
    (
        "AGS Format Rule 19a",
        re.compile(
            r'^Heading "(?P<head>[A-Za-z0-9_]+)" is more than 9 '
            r"characters long\.?$"
        ),
    ...
    (
        "AGS Format Rule 19a",
        re.compile(
            r'^Heading "(?P<head>[A-Za-z0-9_\-]+)" must contain only '
    ...
    (
        "AGS Format Rule 19b",
        re.compile(
            r'^Heading "(?P<head>[A-Za-z0-9_]+)" must be a 4-letter '
    ...
    (
        "AGS Format Rule 20",
        re.compile(
            r"^Declared FILE_FSET sub-folder '(?P<p>[^']+)' is missing on disk\.?$"
        ),
